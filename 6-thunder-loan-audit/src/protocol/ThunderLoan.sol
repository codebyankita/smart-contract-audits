    ...
    OracleUpgradeable
{
    ...
    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    mapping(IERC20 => AssetToken) public s_tokenToAssetToken; //e I think maps the underlying token to it's to assestToken
    ...
    mapping(IERC20 token => bool currentlyFlashLoaning)
        private s_currentlyFlashLoaning; //e prabably a mapping that tells if token is in the middle of flash loan
    ...
    function initialize(address tswapAddress) external initializer {
        ...
        __Oracle_init(tswapAddress); //e using tswap some kind of oracle perhaps?
        s_feePrecision = 1e18; //@written in aderyn 1000
        s_flashLoanFee = 3e15; // 0.3% ETH fee     0003
    }

    //@audit-info where if the natspec??
    function deposit(
        IERC20 token,
        uint256 amount
    ) external revertIfZero(amount) revertIfNotAllowedToken(token) {
        AssetToken assetToken = s_tokenToAssetToken[token]; //e represent share of the pool
        uint256 exchangeRate = assetToken.getExchangeRate(); //e 100e18 USDC * 1e18/ 1e18 (2e18)
        //100e18 *1e18/2e18 =50e18
        //e this should never be 0 beacuse of the asset token conditional
        uint256 mintAmount = (amount * assetToken.EXCHANGE_RATE_PRECISION()) /
            exchangeRate;
        emit Deposit(msg.sender, token, amount);
        assetToken.mint(msg.sender, mintAmount);

        //@audit- high  we have got
        //we shouldent be update the exchange rate here!
    ...
    //e I have 10 AssetToken for USDC, let me get my USDC based on the 10 asset tokenn exchange rate
    function redeem(
        IERC20 token,
        uint256 amountOfAssetToken
    ...
    function flashloan(
        ...
        bytes calldata params // e the parameters to call the receiverAddress with
    ) 
    
    external revertIfZero(amount) 
    revertIfNotAllowedToken(token) {

        AssetToken assetToken = s_tokenToAssetToken[token];//e get this to get the underlying
...
//e making sure the receiver address is smart contract
        if (receiverAddress.code.length == 0) {
            revert ThunderLoan__CallerIsNotContract();
        }

        //e this is probably the feee of flash loan!!
    ...
    //@audit-low you cant use repay to repay a flash loan inside another falsh loan
    ...
    function setAllowedToken(
        ...
        if (allowed) {
            if (address(s_tokenToAssetToken[token]) != address(0)) {
                revert ThunderLoan__AlreadyAllowed(); //@auidt-info revert with token
            }
            //what if they dont have give name?
            ...
            AssetToken assetToken = s_tokenToAssetToken[token];
            delete s_tokenToAssetToken[token]; //q does deleting mapping right
            emit AllowedTokenSet(token, assetToken, allowed);
            return assetToken;
        }
    }
    ...
    // e where is  natspc!!!
    //q answered is this calcating the fees of flash loans
    //a yes!
    ...
    function getCalculatedFee(
        ...
        //1e18?
             ...
             //@audit-high  if the fee is going to be token than value should be refect that
